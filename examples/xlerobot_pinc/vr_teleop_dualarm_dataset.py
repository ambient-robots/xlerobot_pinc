...
class SimpleTeleopArm:
    ...
    def handle_vr_input(self, robot, vr_goal):
        ...
        # If VR says "no motion", also reset filter to kill creep
        if is_stationary_vr_pos:
...
def init_dataset(robot):
    customized_features = {
        "action": {
            ...
            "names": [
                ...
                "left_arm_elbow_flex.pos", "left_arm_elbow_roll.pos", 
                ...
                "right_arm_elbow_flex.pos", "right_arm_elbow_roll.pos", 
    ...
    root = Path("/home/that/.cache/huggingface/lerobot") / f"{HF_REPO_ID}_{timestamp_suffix}"

    dataset = LeRobotDataset.create(
        repo_id=f"{HF_REPO_ID}_{timestamp_suffix}",
        root=str(root),
        fps=FPS,
...
def main():
    ...
    try:
        ...
        vr_thread.start()
        if not vr_monitor.ready_event.wait(timeout=10):
            raise RuntimeError("VR monitor did not become ready within 10s")
        ...
        while recorded_episodes < NUM_EPISODES and not events["stop_recording"]:
            ...
            while timestamp < EPISODE_TIME_SEC:
                ...
                # Start episode timer at first non-none vr goal
        ...
        if dataset:
            dataset.finalize()
            print(f"Collected Dataset {dataset.repo_id}: {dataset.meta.total_episodes} episodes, {dataset.meta.total_frames} frames")
